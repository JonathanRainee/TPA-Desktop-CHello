...
import { addDoc, collection, query, setDoc, where, doc, getDocs, updateDoc, arrayUnion, getFirestore, onSnapshot, startAt, orderBy, endAt} from "firebase/firestore";
...
const Workspace = () => {
    ...
    useEffect(()=>{
        let q = query(workspaceRef,orderBy('title'), startAt(search), endAt(search+"\uf8ff"))
        const searchWs = []
    ...
    <div className="flex flex-col w-0 flex-1 overflow-hidden">
        ...
        <div className="flex-1 px-4 flex justify-between bg-cream">
            ...
            <form className="w-full flex md:ml-0 " action="#" method="GET">
                <label for="search_field" className="sr-only text-black">Search</label>
                ...
                <input onChange={(e)=>setSearch(e.target.value)} id="search_field" className=" w-full h-full pl-8 pr-3 py-2 border-transparent placeholder-black bg-cream placeholder-white focus:outline-none focus:placeholder-black focus:ring-0 focus:border-transparent sm:text-sm" placeholder="Search" type="search" name="search"></input>
                </div>
            </form>
            </div>
        ...
        <main className="flex-1 relative overflow-y-auto focus:outline-none bg-cream pl-4 pt-4" >
            {/* <div>
                <h1 className="text-4xl font-semibold text-gray-900 pl-2">Workspace</h1>
            ...
            <div className=''>
                <h1 className="text-4xl font-semibold text-gray-900 pl-2">Search Result</h1>
                {
                    res.map((item)=>{
                        // return(
                        //     <p>{item.title}</p>
